#!/usr/bin/env python3
"""Validate parent-workspace commit messages against the integration spec."""
...
HEADER_PATTERN = re.compile(
    r"^(feat|fix|refactor|docs|test|build|ci|chore|perf)"
    r"(\([a-z0-9][a-z0-9,-]*\))?: [^\n]{1,72}$"
)
REQUIRED_SECTIONS = (
    "Goal:",
...
def validate_message(message: str) -> tuple[bool, str]:
    ...
    for index, section in enumerate(REQUIRED_SECTIONS):
        ...
        if not block:
            return False, f"section {section} must contain content or 'n/a'"
    return True, ""
...
def main() -> int:
    ...
    print(
        "Allowed types: feat|fix|refactor|docs|test|build|ci|chore|perf",
        file=sys.stderr,
    )
    print("Required sections: Goal, Changes, version-check", file=sys.stderr)
    print(
