...
**SDK (`sdk/`)** tracks only `subprojects/nlohmann_json.wrap`. Consumer integration uses `docs/satellite-plugin-sdk.wrap.example` — not a wrap under `sdk/subprojects/`, so SDK builds never clone the SDK into itself.
